...
- `producer`: Each producer thread:
  ...
  - Terminates after producing a set number of matrices (determined by NUM_OPERATIONS).
...
- `consumer`: Each consumer thread:
  ...
  - Terminates after consuming a set number of matrices (determined by NUM_OPERATIONS).
...
- Creates NUM_PRODUCERS producer threads and NUM_CONSUMERS consumer threads.
...
- Producers generate 3x3 matrices and store them in a buffer of size 5.
- Consumers retrieve and multiply these matrices to produce 3x3 result matrices.
- The process continues until a total of 10 operations are completed.
